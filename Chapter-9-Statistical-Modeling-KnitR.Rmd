...
<<<<<<< HEAD
```{r packages, results = 'hide', fig.show='hide', warning = FALSE, message = FALSE}
=======
```{r packages, results = 'hide', message = FALSE, warning = FALSE}
>>>>>>> 229fe17f2bc25645c4c2f4e3c5e0cde8becf34df
###~~~
# Load R packages
###~~~
...
Here is a string of code that was used to produce a figure. This is also made from ebird data, but instead of starting with someone else's caclulations, we are interacting with the package 'ebirdst', which pulls data Cornell's eBird database.Bogging down markdown documents with lots of long code chunks can impair readibility. There might be times where it is simpler to link to a script that contains analysis code. 
