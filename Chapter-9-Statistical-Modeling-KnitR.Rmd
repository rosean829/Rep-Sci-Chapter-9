...
Here is a string of code that was used to produce a figure. This is also made from ebird data, but instead of starting with someone else's caclulations, we are interacting with the package *ebirdst*, which pulls data Cornell's eBird database. Bogging down markdown documents with lots of long code chunks can impair readibility. There also might be times where it is better to link to a script that contains analysis code, such as: needing the same code for multiple different output documents or to make it easier for other researchers to look for specific parts of your analysis.

(For the next exercise, we will use the following piece of code)

```{r, message=FALSE, eval = FALSE}
# Note eval = FALSE. This code chunk won't actually run. We are going to use an r file with the same code for the next exercise.
#load packages
library(ggplot2)
library(sf)
library(raster)
...
We can link to previously written code locally stored on your computer that you want to pull for an analysis. To do so, we will save that code in its own R file and use the `source` command from *knitR* to call it. This is best in the early stages of your research, as others won't have access to your local files. We have put the code from the code chunk above into a separate r file in the Ch. 9 folder. Now we are going to call it with the `source` function.
...
### Refresher
You can use the argument `include = FALSE` to run the analysis and produce objects that can still be used by other code chunks but will not show up in the output document.
...
Use the `source_url` command from the *devtools* package.
```{r}
